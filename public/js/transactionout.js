...
$(document).ready(function (e) {
  ...
  var table2 = $("#myTable2").DataTable({
    ...
    columns: [
      ...
      { title: "#", className: "w-[3%]" },
      { title: "رقم المستعرض ", className: "w-[5%]" },
      { title: "العنوان", className: "w-[5%]" },
      { title: "المعاملة", className: "w-[10%] text-right" },
    ],
    ordering: false,
    info: false,
      ...
      $("td:eq(4  )", row).html(
        `       <div>
                                <button type="button"
                                    ...
                                    class="   bg-[#8C7155] border text-white  hover:bg-black duration-500 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mt-3  dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                                    استعراض</button>
 ...
 $(".pdfbut").on("click", () => {
  ...
  let mo = $(".mo");
  mo.prepend(`
  <div class="md:w-[50%] w-full my-2 flex justify-between">
  <div class="flex justify-between md:w-[30%] w-[40%]">
    <h1 class="text-[#0D0C22] font-medium">رقم المعاملة</h2>
  <h2 class="text-[#0D0C22CC]">99/4</h2>
  </div>
  
  <div class="flex justify-between md:w-[35%] w-[40%]">
  <h1 class="text-[#0D0C22] font-medium">تاريخها</h2>
  <h2 class="text-[#0D0C22CC]">7 dec 2022</h2>
  </div>
  </div>
  `);
  console.log(mo);
