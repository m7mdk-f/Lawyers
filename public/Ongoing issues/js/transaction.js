$(document).ready(function (e) {
  ...
  var table = $("#myTable").DataTable({
    ...
    ordering: false,
    info: false,
    dom: "<'border  border-2'<'top  '<'flex border-b border-b-2   flex-wrap px-4 py-2 mo  bg-white sm:justify-between justify-center '<'w-auto mt-4'l>>><'relative   w-full mx-auto  overflow-x-auto shadow-md'rt>><'bottom flex relative botton-1 py-4 rounded-b-xl w-full bg-white border-b justify-center items-center ' <'mx-auto mt-2 md:mx-0    custom-page-number' p>>",
    createdRow: function (row, data, dataIndex) {
      $("td:eq(2)", row).html(
        `       <div>
                                <button type="button"
                                    class="  w-1/2 bg-[#8C7155] border font-semibold text-white  hover:bg-black duration-500 focus:ring-4 focus:ring-blue-300  rounded-lg text-sm px-5 py-2.5 mr-2 mt-3  dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                                    استعراض</button>
  ...
  function edittable() {
    ...
    $("#myTable thead tr th").addClass("px-6 py-5 text-center bg-[#E5E5E5] ");
  ...
  edittable();
   ...
   h1.className = 
   "mt-2 text-lg     sm:mb-0 mb-3 text-black";
