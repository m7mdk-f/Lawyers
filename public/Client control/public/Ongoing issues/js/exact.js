  ...
  var table = $("#myTable").DataTable({
    ...
    createdRow: function (row, data, dataIndex) {
      $("td:eq(4)", row).html(
        `       <div>
                                <button type="button"
                                    ...
                                    class=" but2  w-full bg-[#E1A123] border text-white  hover:bg-black duration-500 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mt-3  dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                                    المعاملة</button>
      ...
      $("td:eq(5)", row).html(
        `       <div>
                                <button type="button"
                                    class="w-full  but  border-[#8C7155] text-[#8C7155] hover:text-white border   hover:bg-black duration-500 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mt-3  dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                                    الجلسة</button>
...
$(document).ready(function (e) {
  var table = $("#myTable2").DataTable({
    ...
    columns: [
      ...
      { title: "التاريخ", className: "text-center" },
    ],
    language: {
      search: "",
...
$(document).ready(function (e) {
  ...
  var table3 = $("#myTable3").DataTable({
    ...
    select: true,
    columns: [
      { title: "رقم المستعرض", className: "w-[33%]" },
      { title: " العنوان", className: "w-[33%] " },
      { title: "التفاصيل", className: "w-[33%]" },
    ],
    ordering: false,
    info: false,
    dom: "<'border  border-2'<'top  '<'flex border-b border-b-2   flex-wrap px-4 py-2 mo1  bg-white sm:justify-between justify-center '<'w-auto mt-4'l>>><'relative   w-full mx-auto  overflow-x-auto shadow-md 'rt>><'bottom flex relative botton-1 py-4 rounded-b-xl w-full bg-white border-b justify-center items-center ' <'mx-auto mt-2 md:mx-0    custom-page-number' p>>",
    createdRow: function (row, data, dataIndex) {
      $("td:eq(2)", row).html(
        `       <div>
                                <button type="button"
                                    class="   bg-[#E1A123] border font-semibold text-white  hover:bg-black duration-500 focus:ring-4 focus:ring-blue-300  rounded-lg text-sm px-5 py-2.5 mr-2 mt-3  dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                                    استعراض</button>
  ...
  function edittable() {
    ...
    $("#myTable3 thead").addClass(
      "text-base text-[#0D0C22] font-medium   bg-white py-5 dark:bg-gray-700 dark:text-gray-400 text-right"
    );
    $("#myTable3 thead tr th").addClass("px-6 py-5 text-center bg-[#E5E5E5] ");
    $("#myTable3 tbody").addClass(
      "w-full text-[#0D0C22] h-full   bg-white  dark:text-gray-400 "
    );
    $("#myTable3 tbody tr").addClass(
      "bg-white border-b border-red-400 text-right text-sm text-center font-medium dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
    );
    $("#myTable3  tbody tr th").addClass(
      "px-6 py-4 font-medium text-gray-900 whitespace-nowrap   dark:text-white"
    );
  }
  edittable();
  let mo = document.querySelector(".mo1");
  let divq = document.createElement("div");
  divq.className = "flex justify-between sm:w-[70%] w-full ";
  let h1 = document.createElement("h1");
  h1.innerHTML = `<h1 class="text-[#4F4F4F] text-base mb-2">رقم المستعرض:</h1>44/3`;
  h1.className = "mt-2 text-lg     sm:mb-0 mb-3 text-black";

  let h2 = document.createElement("h1");

  h2.innerHTML = `<h1 class="text-[#4F4F4F] text-base mb-2">الموضوع:</h1>استدعاء....`;
  h2.className = "mt-2 text-lg     sm:mb-0 mb-3 text-black";

  let h3 = document.createElement("h1");

  h3.innerHTML = `<h1 class="text-[#4F4F4F] text-base mb-2">تاريخها:</h1>9 Dec 2023`;
  h3.className = "mt-2 text-lg     sm:mb-0 mb-3 text-black";
  ...
  $("#myTable3").on("draw.dt", function () {
    rplacebutton();
    trch = document.querySelectorAll("#myTable3 tbody .odd");
    console.log(trch);
    edittable();
  });
  ...
  let dataTables_length = $(".dataTables_length");
  console.log(dataTables_length.eq(1));
  dataTables_length.css("float", "none");
  ...
  rplacebutton();
  function rplacebutton() {
    let $paginate = $("#myTable3_paginate");
    $paginate.addClass("relative");
    let $prevButton = $paginate.find(".paginate_button.previous");
    let $nextButton = $paginate.find(".paginate_button.next");
    let $customButton;
    ...
    if ($prevButton.hasClass("disabled")) {
      $customButton =
        $(`<button id="privbut" type="button" class=" text-lg ml-2 disp " aria-controls="myTable3" tabindex="0"><svg width="8" height="12" viewBox="0 0 8 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M6.99481 7.00495L2.40181 11.5979C2.31023 11.6914 2.20102 11.7658 2.08051 11.8168C1.96001 11.8678 1.83059 11.8943 1.69974 11.895C1.56889 11.8957 1.43921 11.8704 1.31819 11.8206C1.19718 11.7708 1.08723 11.6976 0.994704 11.6051C0.902179 11.5125 0.828914 11.4026 0.779145 11.2816C0.729376 11.1605 0.704092 11.0309 0.704753 10.9C0.705414 10.7692 0.732008 10.6398 0.782996 10.5192C0.833985 10.3987 0.908356 10.2895 1.00181 10.1979L4.90181 6.29795L1.00181 2.39795C0.819248 2.21166 0.717578 1.96085 0.718896 1.70002C0.720213 1.43919 0.824411 1.18942 1.00885 1.00498C1.19328 0.820548 1.44305 0.716349 1.70388 0.715032C1.96471 0.713715 2.21552 0.815383 2.40181 0.997947L6.99481 5.58995C7.18228 5.77748 7.2876 6.03178 7.2876 6.29695C7.2876 6.56211 7.18228 6.81642 6.99481 7.00395L6.99481 7.00495Z" />
                            </svg>
                            </button>`);
    } else {
      $customButton =
        $(`<button id="privbut" type="button" class=" text-lg ml-2 actbut " aria-controls="myTable3" tabindex="0"><svg width="8" height="12" viewBox="0 0 8 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M6.99481 7.00495L2.40181 11.5979C2.31023 11.6914 2.20102 11.7658 2.08051 11.8168C1.96001 11.8678 1.83059 11.8943 1.69974 11.895C1.56889 11.8957 1.43921 11.8704 1.31819 11.8206C1.19718 11.7708 1.08723 11.6976 0.994704 11.6051C0.902179 11.5125 0.828914 11.4026 0.779145 11.2816C0.729376 11.1605 0.704092 11.0309 0.704753 10.9C0.705414 10.7692 0.732008 10.6398 0.782996 10.5192C0.833985 10.3987 0.908356 10.2895 1.00181 10.1979L4.90181 6.29795L1.00181 2.39795C0.819248 2.21166 0.717578 1.96085 0.718896 1.70002C0.720213 1.43919 0.824411 1.18942 1.00885 1.00498C1.19328 0.820548 1.44305 0.716349 1.70388 0.715032C1.96471 0.713715 2.21552 0.815383 2.40181 0.997947L6.99481 5.58995C7.18228 5.77748 7.2876 6.03178 7.2876 6.29695C7.2876 6.56211 7.18228 6.81642 6.99481 7.00395L6.99481 7.00495Z" />
                        </svg>
                        </button>`);
    }
    ...
    if ($nextButton.hasClass("disabled")) {
      $nectcusButton =
        $(`<button type="button" class=" text-lg  mr-2 disp" aria-controls="myTable3" tabindex="0"><svg width="8" height="12" viewBox="0 0 8 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path fill-rule="evenodd" clip-rule="evenodd" d="M1.00483 7.00493C0.817362 6.8174 0.712047 6.56309 0.712047 6.29793C0.712047 6.03277 0.817362 5.77846 1.00483 5.59093L5.59783 0.997929C5.78412 0.815366 6.03493 0.713697 6.29576 0.715014C6.55659 0.716331 6.80636 0.82053 6.9908 1.00497C7.17523 1.1894 7.27943 1.43917 7.28075 1.7C7.28207 1.96083 7.1804 2.21164 6.99783 2.39793L3.09783 6.29793L6.99783 10.1979C7.1804 10.3842 7.28207 10.635 7.28075 10.8959C7.27943 11.1567 7.17523 11.4065 6.9908 11.5909C6.80636 11.7753 6.55659 11.8795 6.29576 11.8808C6.03493 11.8822 5.78412 11.7805 5.59783 11.5979L1.00483 7.00493Z" />
                                        </svg>
                                        </button>`);
    } else {
      $nectcusButton =
        $(`<button type="button" class=" text-lg  mr-2 actbut" aria-controls="myTable3" tabindex="0"><svg width="8" height="12" viewBox="0 0 8 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path fill-rule="evenodd" clip-rule="evenodd" d="M1.00483 7.00493C0.817362 6.8174 0.712047 6.56309 0.712047 6.29793C0.712047 6.03277 0.817362 5.77846 1.00483 5.59093L5.59783 0.997929C5.78412 0.815366 6.03493 0.713697 6.29576 0.715014C6.55659 0.716331 6.80636 0.82053 6.9908 1.00497C7.17523 1.1894 7.27943 1.43917 7.28075 1.7C7.28207 1.96083 7.1804 2.21164 6.99783 2.39793L3.09783 6.29793L6.99783 10.1979C7.1804 10.3842 7.28207 10.635 7.28075 10.8959C7.27943 11.1567 7.17523 11.4065 6.9908 11.5909C6.80636 11.7753 6.55659 11.8795 6.29576 11.8808C6.03493 11.8822 5.78412 11.7805 5.59783 11.5979L1.00483 7.00493Z" />
                                        </svg>
                                        </button>`);
    }
